...
SANDESH_SRC = sandesh/sandesh.tex sandesh/sandesh.bib
SANDESH_PDF = sandesh/sandesh.pdf
DOC_SRC     = doc/metropolistheme.dtx
DOC_PDF     = doc/metropolistheme.pdf

CTAN_CONTENT = README.md $(INS) $(PACKAGE_SRC) $(DOC_SRC) $(DOC_PDF) $(DEMO_SRC) $(DEMO_PDF)  $(SANDESH_SRC) $(SANDESH_PDF)
...
.PHONY: all sty doc demo clean install uninstall ctan clean-cache clean-sty ctan-version docker-run docker-build docker-rm sandesh 

all: sty doc sandesh demo 
...
$(SANDESH_PDF): $(SANDESH_SRC) $(PACKAGE_STY) | clean-cache $(CACHE_DIR)
	@cd $(dir $(SANDESH_SRC)) && $(COMPILE_TEX) $(notdir $(SANDESH_SRC))
	@cp $(CACHE_DIR)/$(notdir $(SANDESH_PDF)) $(SANDESH_PDF)
